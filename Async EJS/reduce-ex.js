...
function vowelCount(str){
    let arrVowel = ['a', 'i', 'o', 'u', 'e'];
    return arrVowel.reduce((accumulator, nextValue) => {
        let pos = -1;
        let count = 0;
        while ((pos = str.toLowerCase().indexOf(nextValue, pos + 1)) !== -1) {
            count++;
            accumulator[nextValue] = count;
        }
        return accumulator;
    }, {})
}
...
var NextArr = [{name: 'Elie'}, {name: 'Tim'}, {name: 'Matt'}, {name: 'Colt'}];
...
console.log(addKeyAndValue(NextArr, 'title', 'Instructor'));
